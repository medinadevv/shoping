...
export const BasketReducer = (state = initialState, action  ) => {
    switch (action.type) {
        ...
        case "DELETE_BASKET" :
            let del = state.basket.find(el => el.id === action.payload.id)
            if (del) {
                return {
                    ...state,
                    basket: state.basket.filter((el) => el.id !== del.id ? {...el, delId: el.delId - 1} : 0)
                }
            } else {
                return {...state, basket: [...state.basket, {...action.payload, delId: 1}]}
            }
        case "DEC_BASKET" : {
